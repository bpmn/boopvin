<?php

/*
 * To change this template, choose Tools | Templates
 * and open the template in the editor.
 */

$english = array( 
   'contact'=> 'contact us',
   'contact:thanks'=>" Thanks for contacting us!", 
   'contact:merci'=>" Thank you!",
   'contact:required'=> 'required fields',
   'contact:antispam'=> "Please answer the anti-spam question",
   'contact:question'=> "Anti-spam question",
   'contact:message'=> "Message:",
   'contact:email'=> "Email Address*:", 
   'contact:name'=> "Your Full Name*:",
   'contact:validate:name'=> "Please provide your name",
   'contact:validate:email'=>"Please provide your email address",
   'contact:validate:bad_email'=>"Please provide a valid email address",
   'contact:validate:email'=>"The message is too long!(more than 2KB!)",
   'contact:validate:antispam'=>"Please answer the anti-spam question"
   );

add_translation("en", $english);